...
// var PRESET_URL = "https://92ecc4d1.ngrok.io";
var PRESET_URL = "http://localhost:8080";
...
if (document.referrer === PRESET_URL + "/enroll") {
...
function stopRecording() {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
            ...
            //document.getElementById('votefortxt').innerHTML = xhr.response;

            if (xhr.response !== "") {
                // alert('ยืนยันการลงคะแนนให้ ' + xhr.response + ' หรือไม่?');

                Swal.fire({
                    title: 'ยืนยันการลงคะแนนเสียง',
                    text: "ยืนยันการลงคะแนนให้ " + xhr.response + ' หรือไม่?',
                    icon: 'warning',
    ...
    xhr.open("POST", "/votenow", true);
    xhr.send(soundBlob);

    console.log("Your http message has been sent.(voting)");
}
...
document.querySelector('#close_button_accept').addEventListener('click', function () {
    if (number_of_attempts < 0) {
        if (document.referrer === PRESET_URL + "/auth") {
            hideElement('#acceptMessage');
            showElement('#vadMessage');
            hideElement('#passphraseMessage');
